...
__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(MEM_SDRAM_BASE)));  //外部SDRAM内存池
#elif (defined (__ICCARM__))
#pragma pack(4)
__no_init uint8_t mem1base[MEM1_MAX_SIZE]; //内部SRAM内存池   
...
uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(MEM_SDRAM_BASE+MEM2_MAX_SIZE)));  //外部SDRAM内存池MAP
#elif (defined (__ICCARM__))
...
{  
  ...
  //printf("free:%d,%x\r\n",memx,ptr);
  vPortEnterCritical();		//进入临界区(无法被中断打断)     
  offset=(uint32_t)ptr-(uint32_t)mallco_dev.membase[memx];  
  my_mem_free(memx,offset);	//释放内存     
  vPortExitCritical();			//退出临界区(可以被中断打断) 
}  
//分配内存(外部调用)
//memx:所属内存块
//size:内存大小(字节)
